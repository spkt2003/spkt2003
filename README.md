# Hi there, I'm Phu Thanakorn Pinsuk! 👋

**IT Student @ HCU | Web Developer & IoT Enthusiast**

ผมเป็นนักศึกษาจากมหาวิทยาลัยหัวเฉียวเฉลิมพระเกียรติ ที่มีความสนใจด้านการพัฒนา Web Application และการนำระบบ IoT มาประยุกต์ใช้แก้ปัญหาในชีวิตจริง ปัจจุบันกำลังสนุกกับการสร้างโปรเจกต์ต่างๆ และ **กำลังมองหาสถานที่ทำสหกิจศึกษา (Co-op)** เพื่อพัฒนาทักษะและเรียนรู้การทำงานในสภาพแวดล้อมจริงครับ 🚀

---

### 🛠 Tech Stack & Tools
<p>
  <img src="https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" />
  <img src="https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white" />
  <img src="https://img.shields.io/badge/SQLite-07405E?style=for-the-badge&logo=sqlite&logoColor=white" />
  <img src="https://img.shields.io/badge/JSON-000000?style=for-the-badge&logo=json&logoColor=white" />
  <img src="https://img.shields.io/badge/Node--RED-8F0000?style=for-the-badge&logo=nodered&logoColor=white" />
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/IoT_(ESP32)-000000?style=for-the-badge&logo=espressif&logoColor=white" />
</p>

---

### 🚀 Featured Projects
รวบรวมผลงานเด่นที่ผมได้ลงมือทำและพัฒนาขึ้นมาครับ:

* 🏠 **Homy**
  * **Description:** แพลตฟอร์มเชื่อมโยงระหว่างเจ้าของบ้านและผู้รับดูแลบ้าน (Homy) โดยเน้นไปที่ความปลอดภัย ความน่าเชื่อถือ และระบบการจับคู่ที่ตอบโจทย์
  * **Tech Stack:** * **Frontend:** Next.js, React, TypeScript, Tailwind CSS, shadcn/ui, Lucide React
    * **Backend / Database:** Supabase

* 📦 **System ยืมคืน (Equipment Borrow/Return System)**
  * **Description:** ระบบ Desktop Application สำหรับจัดการยืม-คืนอุปกรณ์ มาพร้อมหน้าจอ GUI สไตล์ Modern Dark-mode และมีระบบสร้าง QR Code สำหรับจัดการเรื่องค่าปรับ
  * **Tech Stack:** * **Language & GUI:** Python, CustomTkinter (Tkinter)
    * **Tools & Libraries:** Pillow (PIL), qrcode, python-dateutil
    * **Database:** SQLite

* 🚑 **CPR-Health-Robotic**
  * **Description:** เกม Interactive สำหรับฝึกทักษะการทำ CPR โดยใช้กล้อง Webcam ตรวจจับสัญญาณแสงไฟจากหุ่นจำลอง CPR ผู้เล่นจะต้องรักษาระดับการกดหน้าอกให้ไฟสีเขียวสว่างต่อเนื่องเป็นเวลา 1 นาที
  * **Tech Stack:** Node-RED, Webcam Detection

* 🎮 **2D-HTML-GAME-Project (Wordle Clone)**
  * **Description:** โปรเจกต์โคลนเกมทายคำศัพท์ยอดฮิตของ The New York Times (Wordle) ในรูปแบบ Web-based Game 2D โดยมีการดึงข้อมูลคลังคำศัพท์จากไฟล์ JSON มาใช้ในระบบสุ่มและตรวจสอบความถูกต้องของคำ
  * **Tech Stack:** HTML, CSS, JavaScript, JSON

---

### 📫 How to reach me
* **Email:** tkpss2003@gmail.com
* **Resume:** [คลิกเพื่อดู Resume ของผม](ใส่ลิงก์ Google Drive หรือเว็บไซต์ Portfolio ตรงนี้)
